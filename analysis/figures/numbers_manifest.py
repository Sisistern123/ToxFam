...
def _emit_latex_macros(out: dict, path) -> None:
    ...
    manifest and the prose can never silently disagree. Copy it into the manuscript
    repo (like the figure PDFs) and ``\input`` it; each macro expands to exactly the
    number the captions/body used to hard-code. Macro names are letters-only (LaTeX
    forbids digits in \newcommand names).
    """
    mcc = out["mcc"]
    macros = {
    ...
    header = [
        ...
        "% derived from results_numbers.json. Regenerate the manifest and copy this file",
        r"% into the manuscript repo (like the figure PDFs), then \input it in main.tex.",
        "",
    ]
