...
def domain_accs(path):
    return {line[1:].strip().split()[0] for line in open(path) if line.startswith(">")}


def main():
    seen = fasta_accs(TOXDL2 / "protein_sequences/train.fasta") | domain_accs(TOXDL2 / "domain_data/valid.domain")
