...
def main() -> None:
    ...
    fam = (
        per_family_mcc_difference(nn, hbi, class_list=classes)
        .dropna(subset=["mcc_a", "mcc_b"])
        .sort_values("mcc_a")
    ...
    ax.scatter(
        ...
        color=nn_col,
        marker="s",
        edgecolor=METHOD_DARK["nn_combined_run"],
        linewidth=0.5,
        alpha=0.7,
        label=nn_lbl,
        zorder=3,
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(
        [_fam_label(n, s) for n, s in zip(fam["family"], fam["support"])], fontsize=7
    )
