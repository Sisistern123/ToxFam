...
def _acc_panel(ax, preds, mask_fn, ylim, title, letter, *, prior=None):
    ...
    if prior is not None:  # drawn on top of the bars so the ceiling is unmistakable
        ...
        ax.text(
            ...
            prior + 0.002,
            f"non-toxin prior {prior * 100:.2f}%",
            ha="right",
            va="bottom",
            fontsize=6.5,
            color="#444444",
...
def main() -> None:
    ...
    fig, (axA, axB) = plt.subplots(
        2, 1, figsize=(SINGLE_COL, 4.7), layout="constrained"
    )
    n_tox = int(
        toxin_mask(preds["nn_combined_run"]).sum()
    )  # from data, not a frozen literal
    _acc_panel(
        axA, preds, toxin_mask, (0.70, 1.0), f"Toxin-only accuracy ($n$={n_tox})", "A"
    )
    prior = float(
        (preds["nn_combined_run"]["actual_label"].str.lower() == "nontox").mean()
    )
