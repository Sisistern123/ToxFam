...
def compute() -> dict[str, int | float]:
    ...
    raw_nontox = pd.read_csv(raw_dir() / "nontox.tsv", sep="\t")
    ...
    v: dict[str, int | float] = {
        ...
        # Non-toxins that DO carry a UniProt family annotation. Methods needs it to say
        # why the missing-family filter is toxin-only: it is not that non-toxins have no
        # family, it is that the pipeline overwrites whatever family they have with the
        # single "nontox" label (preprocessing.load_and_prepare_raw).
        "NontoxWithFamily": int(raw_nontox["Protein families"].notna().sum()),
...
def emit(values: dict[str, int | float], path) -> None:
    ...
    order = [
        ...
        (
            ...
            ["DropNoFam", "NontoxWithFamily", "FamTox", "FamTotal", "NormLabels"],
        ),
    ...
    # `order` is what actually gets written, so a value computed above and forgotten here
    # would vanish with no error and the manuscript macro would simply be undefined at
    # build time. Fail loudly instead.
    missing = set(values) - {n for _, names in order for n in names}
    if missing:
        raise SystemExit(
            "computed but not emitted: "
            + ", ".join(sorted(missing))
            + " -- add each to `order` (or stop computing it)"
        )
