...
The confident-error verdict panel uses the completed expert curation: all 255
confident errors (calibrated confidence >= 0.8; test 85 + val 82 + 1/3 train 88),
generated blind against the pinned split manifest by
``scripts/build_confident_error_curation.py`` and returned fully answered. It
replaces the earlier n=63 adjudication, which was judged against a split since
...
def _load():
    ...
    # Split and confidence come from the curation key, which was written from the
