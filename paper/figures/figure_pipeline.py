...
def _build() -> plt.Figure:
    ...
    )  # full-bleed (save_fig has no tight bbox)
    ax.set_xlim(0, 124)
    ax.set_ylim(-23, 26)
    ax.axis("off")
    ...
    HEAD_TOP = (
        GAP + 13.0 + 8.0
    )  # common header TOP line; va="top" aligns all first lines
    ...
    ax.add_patch(
        FancyArrowPatch(
            fork, tox_end, arrowstyle="-|>", mutation_scale=10, lw=1.2, color=TOX_DK
        )
    )
    ax.add_patch(
        FancyArrowPatch(
            fork, non_end, arrowstyle="-|>", mutation_scale=10, lw=1.2, color=NON_DK
        )
    )
    ...
    yin = 3.4 / (26 - (-23))
    ang = lambda p0, p1: math.degrees(
        math.atan2((p1[1] - p0[1]) * yin, (p1[0] - p0[0]) * xin)
    )
    ax.text(
        22.6,
        5.5,
    ...
    for i in range(len(levels) - 1):
        ...
        ax.fill(
            [x0, x1, x1, x0],
            [GAP, GAP, GAP + ht(levels[i + 1][1]), GAP + ht(levels[i][1])],
            color=TOX,
            alpha=0.16,
            lw=0,
        )
        ax.fill(
            [x0, x1, x1, x0],
            [-GAP, -GAP, -GAP - hn(levels[i + 1][2]), -GAP - hn(levels[i][2])],
            color=NON,
            alpha=0.16,
            lw=0,
        )
    ...
    for i, (lab, tox, non) in enumerate(levels):
        ...
        ax.add_patch(
            plt.Rectangle(
                (x - BARW / 2, GAP), BARW, ht(tox), fc=TOX, ec="white", lw=0.8
            )
        )
        ax.text(
            x,
    ...
    for i, vt in enumerate(vocab_txt):
        ...
        if i < len(vocab_txt) - 1:
            ax.annotate(
                "",
                xy=(xs[i + 1] - BARW / 2 - 0.8, VOCAB_Y),
                xytext=(xs[i] + BARW / 2 + 0.8, VOCAB_Y),
    ...
    ax.add_patch(
        plt.Rectangle((px, py), pw * levels[0][1] / tot_raw, 1.4, fc=TOX, ec="none")
    )
    ax.add_patch(
        plt.Rectangle(
            (px + pw * levels[0][1] / tot_raw, py),
            pw * levels[0][2] / tot_raw,
            1.4,
            fc=NON,
            ec="none",
        )
