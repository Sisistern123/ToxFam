...
def _require_matching_toxin_cohort(out: dict) -> None:
    r"""Refuse to emit \HbiNontoxBestHit if it and \NumTox count different cohorts.

    The Discussion prints the two as one fraction ("68 of the 515 test toxins"), but
    they are counted on different frames: the numerator on the HBI predictions, the
    denominator on the combined model's. They agree only while both cover the same
    test proteins, and the NN path drops rows lacking an embedding. A divergence would
    make that fraction quietly wrong rather than obviously absent, so it fails here.
    """
    n_tox_hbi = out["hbi_nontoxin_best_hit"]["n_toxins"]
    if n_tox_hbi != out["toxin_only_n"]:
        raise SystemExit(
            f"toxin cohort mismatch: HBI has {n_tox_hbi} test toxins, the combined "
            f"model {out['toxin_only_n']}. \\HbiNontoxBestHit is cited over \\NumTox "
            "in the Discussion, so emitting both would misstate the denominator. "
            "Give the statistic its own denominator macro before regenerating."
        )


# paper.stats.macro_mcc_by_support labels each record with its own `group` field, so the
# macro stem is keyed off that rather than off the record's position in the list. The
# positional form passed `strict=True` on length while silently swapping the two numbers
# if the rows were ever reordered -- the same class of drift these macros exist to end.
# The keys pin support_threshold=5: change it and this raises KeyError instead of quietly
# labelling the rare-family number "WellSupported".
_SUPPORT_GROUPS = {"support>5": "WellSupported", "support<=5": "Rare"}

_SUPPORT_FIELDS = (
    ("MacroMcc{grp}Toxfam", "macro_mcc_a", "{:.3f}"),
    ("MacroMcc{grp}Hbi", "macro_mcc_b", "{:.3f}"),
    ("Num{grp}Families", "n_families", "{}"),
    ("Num{grp}Toxins", "n_sequences", "{}"),
)


def _support_macros(records: list[dict]) -> dict[str, str]:
    """Support-stratified macro-MCC macros. ``_a`` is ToxFam (emb+tax), ``_b`` is HBI."""
    macros = {}
    for rec in records:
        grp = _SUPPORT_GROUPS[rec["group"]]
        for name, key, fmt in _SUPPORT_FIELDS:
            macros[name.format(grp=grp)] = fmt.format(rec[key])
    return macros
...
def _emit_latex_macros(out: dict, path) -> None:
    ...
    macros = {
        ...
        **_support_macros(out["macro_mcc_by_support"]),
    }
    header = [
...
def main() -> None:
    ...
    out = {
        ...
        # Realised, test-split counterpart of the dataset-wide best-hit floor that
