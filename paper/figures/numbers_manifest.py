...
def _emit_latex_macros(out: dict, path) -> None:
    ...
    macros = {
        ...
        # class of drift impossible. `_a` is ToxFam (emb+tax), `_b` is HBI.
        **{
            f"MacroMcc{grp}{meth}": f"{rec[key]:.3f}"
            for rec, grp in zip(
                out["macro_mcc_by_support"], ("WellSupported", "Rare"), strict=True
            )
            for meth, key in (("Toxfam", "macro_mcc_a"), ("Hbi", "macro_mcc_b"))
        },
        **{
            f"Num{grp}{what}": str(rec[key])
            for rec, grp in zip(
                out["macro_mcc_by_support"], ("WellSupported", "Rare"), strict=True
            )
            for what, key in (("Families", "n_families"), ("Toxins", "n_sequences"))
        },
    }
    header = [
